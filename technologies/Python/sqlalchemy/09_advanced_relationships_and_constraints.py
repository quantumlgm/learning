...
from sqlalchemy.orm import DeclarativeBase, Mapped, contains_eager, mapped_column, relationship, selectinload, sessionmaker
...
def age_select():
    with sessionf.begin() as session:
        stmt = (
            select(WorkersOrm)            
            .options(selectinload(WorkersOrm.resumes_adult))
        )
